...
import { Button, Calendar, PopoverTrigger, Popover, PopoverContent, DateValue } from "@heroui/react"
        ...
        <div className="pt-6">
          <div className={"grid gap-2"}>
            <Popover>
              ...
              >
                <Calendar 
                  focusedValue={date?.from && null}
                  // selected={date && null} 
                  onChange={(value: DateValue) => {
                    if (Array.isArray(value)) {
                      setDate({ from: value[0], to: value[1] }); // Convert to DateRange
                    } else {
                      setDate(undefined); // Handle case where value is not a range
                    }
                  }}
                  visibleMonths={2} 
                  // min={(day: number) => {
                  //   const selectedDate = new Date(day); // Convert day to Date
                  //   return selectedDate <= addDays(new Date(), 1); // Compare Dates
                  // }}
                />
              </PopoverContent>
            </Popover>
